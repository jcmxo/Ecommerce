# 🔄 Diagrama de Flujo Completo del Sistema

## 📊 Flujo Principal

```
┌─────────────────────────────────────────────────────────────────┐
│                    SISTEMA E-COMMERCE BLOCKCHAIN                │
└─────────────────────────────────────────────────────────────────┘

┌─────────────────┐
│  1. INICIO      │
│                 │
│  - Anvil        │
│  - Contratos    │
│    desplegados  │
└────────┬────────┘
         │
         ├─────────────────────────────────────────────────┐
         │                                                 │
         ▼                                                 ▼
┌─────────────────┐                            ┌─────────────────┐
│  2. CLIENTE     │                            │  3. EMPRESA     │
│                 │                            │                 │
│  Wallet 1       │                            │  Wallet 2       │
│  0xf39f...2266  │                            │  0x1234...5678  │
└────────┬────────┘                            └────────┬────────┘
         │                                             │
         │                                             │
         ▼                                             ▼
┌─────────────────┐                            ┌─────────────────┐
│ COMPRA EURT    │                            │ REGISTRA        │
│ (Stripe)       │                            │ EMPRESA         │
│                │                            │                 │
│ localhost:6001 │                            │ localhost:6003  │
└────────┬────────┘                            └────────┬────────┘
         │                                             │
         │                                             │
         │                                             ▼
         │                                  ┌─────────────────┐
         │                                  │ AGREGA          │
         │                                  │ PRODUCTOS       │
         │                                  │                 │
         │                                  │ - Nombre        │
         │                                  │ - Precio        │
         │                                  │ - Stock         │
         │                                  └────────┬────────┘
         │                                           │
         │                                           │
         │                                           │
         ▼                                           │
┌─────────────────┐                                 │
│ VISITA TIENDA   │                                 │
│                 │                                 │
│ localhost:6004  │                                 │
│                 │                                 │
│ - Ver productos │                                 │
│ - Agregar carrito│                                │
└────────┬────────┘                                 │
         │                                          │
         │                                          │
         ▼                                          │
┌─────────────────┐                                 │
│ CHECKOUT        │                                 │
│                 │                                 │
│ - Crear factura │                                 │
│ - Limpiar carrito│                                │
└────────┬────────┘                                 │
         │                                          │
         │                                          │
         ▼                                          │
┌─────────────────┐                                 │
│ PASARELA PAGOS  │                                 │
│                 │                                 │
│ localhost:6002  │                                 │
│                 │                                 │
│ Paso 1: Aprobar │                                 │
│ Paso 2: Pagar   │                                 │
└────────┬────────┘                                 │
         │                                          │
         │                                          │
         │    ┌─────────────────────────────────────┘
         │    │
         ▼    ▼
    ┌─────────────────┐
    │   BLOCKCHAIN    │
    │                 │
    │ - Transfer EURT │
    │ - Actualizar    │
    │   stock         │
    │ - Marcar        │
    │   factura       │
    └────────┬────────┘
             │
             │
             ├─────────────────┬─────────────────┐
             │                 │                 │
             ▼                 ▼                 ▼
    ┌─────────────┐  ┌─────────────┐  ┌─────────────┐
    │ CLIENTE     │  │ EMPRESA     │  │ HISTORIAL   │
    │             │  │             │  │             │
    │ Balance:    │  │ Balance:    │  │ Ver pedidos │
    │ -10 EURT    │  │ +10 EURT    │  │ Ver detalles│
    │             │  │             │  │             │
    │ ✅          │  │ ✅          │  │ ✅          │
    └─────────────┘  └─────────────┘  └─────────────┘
```

---

## 📱 Flujo Detallado de Compra

```
CLIENTE                          BLOCKCHAIN                       EMPRESA
   │                                │                                │
   │ 1. Ver productos               │                                │
   │───────────────────────────────>│                                │
   │                                │                                │
   │ 2. Agregar al carrito          │                                │
   │───────────────────────────────>│                                │
   │                                │                                │
   │                                │ 3. CartItemAdded (event)       │
   │                                │───────────────────────────────>│
   │                                │                                │
   │ 4. Checkout (createInvoice)    │                                │
   │───────────────────────────────>│                                │
   │                                │                                │
   │                                │ 5. InvoiceCreated (event)      │
   │                                │───────────────────────────────>│
   │                                │                                │
   │ 6. Redirige a pasarela         │                                │
   │───────────────────────────────>│                                │
   │                                │                                │
   │ 7. Aprobar tokens (approve)    │                                │
   │───────────────────────────────>│                                │
   │                                │                                │
   │ 8. Procesar pago               │                                │
   │    (processPayment)            │                                │
   │───────────────────────────────>│                                │
   │                                │                                │
   │                                │ 9. Transferir EURT             │
   │                                │    Cliente → Empresa           │
   │                                │───────────────────────────────>│
   │                                │                                │
   │                                │ 10. PaymentProcessed (event)   │
   │                                │───────────────────────────────>│
   │                                │                                │
   │ 11. Ver historial              │                                │
   │───────────────────────────────>│                                │
   │                                │                                │
   │ 12. Factura marcada como       │                                │
   │     "Pagada"                   │                                │
   │<───────────────────────────────│                                │
```

---

## 🎯 Componentes del Sistema

### 1. Smart Contracts
- **EuroToken**: Token ERC20 para pagos
- **Ecommerce**: Contrato principal con toda la lógica

### 2. Aplicaciones Web
- **Compra Stablecoin** (6001): Comprar EURT con Stripe
- **Pasarela de Pagos** (6002): Procesar pagos de facturas
- **Web Admin** (6003): Gestionar empresa y productos
- **Web Customer** (6004): Tienda online para clientes

### 3. Blockchain Local
- **Anvil**: Red blockchain local en puerto 8545

---

## 🔐 Roles y Permisos

### Cliente (Wallet 1)
- ✅ Ver productos
- ✅ Agregar al carrito
- ✅ Crear facturas
- ✅ Pagar facturas
- ✅ Ver historial

### Empresa (Wallet 2)
- ✅ Registrar empresa
- ✅ Agregar productos
- ✅ Actualizar stock
- ✅ Ver facturas recibidas
- ✅ Recibir pagos

---

## 💰 Flujo de Tokens

```
INICIO: Cliente tiene 1,000,000 EURT
        Empresa tiene 0 EURT

1. Cliente compra producto de 10 EURT
   │
   ├─> Cliente: 1,000,000 - 10 = 999,990 EURT ✅
   │
   └─> Empresa: 0 + 10 = 10 EURT ✅

RESULTADO: Tokens transferidos correctamente
```

---

## 🚨 Errores Comunes y Soluciones

1. **Misma wallet para cliente y empresa**
   - ❌ Balance no cambia
   - ✅ Usar 2 wallets diferentes

2. **Factura ya pagada**
   - ❌ Error al intentar pagar
   - ✅ Verificar estado antes de pagar

3. **Stock insuficiente**
   - ❌ Error al crear factura
   - ✅ Actualizar stock primero

4. **Balance insuficiente**
   - ❌ Error al pagar
   - ✅ Comprar más EURT primero

